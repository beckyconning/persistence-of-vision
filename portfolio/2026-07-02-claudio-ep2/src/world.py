...
def panel_custom(lines, w=6.2, h=2.7, name="panel2"):
    """Generic prompt panel. lines = [(text, px, color, y_offset), ...]"""
    root = Node(name)
    back = root.add(Node(name + "_back"))
    back.mesh(*merge(
        tf(box(w, h, 0.12, PANEL_INK), pos=(0, 0, 0.02)),
        tf(box(w - 0.2, h - 0.2, 0.12, PANEL_BG), pos=(0, 0, -0.04)),
    ))
    for i, (txt, px, col, dy) in enumerate(lines):
        t = text_node(txt, px=px, color=col)
        t.pos[:] = (0, dy, -0.12)
        root.add(t)
    root.visible = False
    return root
...
def magenta_tiles(n_tiles=42, x0=-6, x1=18, z0=-4, z1=14, seed=7):
    """Missing-texture tiles that flicker on during corruption."""
    rng = np.random.default_rng(seed)
    tiles = []
    for i in range(n_tiles):
        t = Node(f"mtile{i}", pos=(rng.uniform(x0, x1), 0.015 + i * 0.0004,
                                   rng.uniform(z0, z1)))
        col = (255, 0, 255) if i % 2 == 0 else (16, 16, 16)
        # checker of 4 sub-quads magenta/black
        m = merge(
            tf(box(1.0, 0.02, 1.0, col), pos=(-0.5, 0, -0.5)),
            tf(box(1.0, 0.02, 1.0, (16, 16, 16) if col[0] > 100 else (255, 0, 255)),
               pos=(0.5, 0, -0.5)),
            tf(box(1.0, 0.02, 1.0, (16, 16, 16) if col[0] > 100 else (255, 0, 255)),
               pos=(-0.5, 0, 0.5)),
            tf(box(1.0, 0.02, 1.0, col), pos=(0.5, 0, 0.5)),
...
def flying_cap():
    """Claudio's cap, detached (for the hit) — reuses the cap geometry."""
    from claudio import CAP, EYE_W
    n = Node("flycap")
    m = merge(
...
def claudio_fixed_sign():
    n = Node("claudiofixed")
    t = text_node("CLAUDIO FIXED ✓", px=0.14, color=(255, 255, 255), depth=0.06)
    t.pos[:] = (0, 0.3, 0)
    n.add(t)
    n.visible = False
    return n
