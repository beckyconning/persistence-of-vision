...
| ![](images/raymarch4.png) | **Stone, reflected** (second bounce) | The technique pushed furthest: a **polished floor** — where the primary ray hits the ground, reflect it and **march again**, shading that second hit and blending by a Fresnel weight (grazing → more mirror). The stone now appears *reflected* in the floor — real GI beyond direct light. The session's strongest image. | [raymarch4.py](src/raymarch4.py) |
