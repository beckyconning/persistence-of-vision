"""Laugh Lines: a face drawn only with the creases a laugh makes. No outline, no hair, no shading.
...
tilt = np.radians(-9)           # head back a little, the way a real laugh goes
...
def half(k):
    lid = [((-4.0, -0.95), (-2.6, -0.95 - 1.4 * k), (-1.35, -0.95), 0.22 + 0.12 * k)]   # upper lid, closing
    low = [((-3.55, -0.5 - 0.12 * k), (-2.6, 0.15 - 0.35 * k), (-1.65, -0.5 - 0.12 * k), 0.16)]
    feet = [((-4.15, -0.9), (-4.15 - 0.65 * k, -0.9 - 0.45 * k), (-4.15 - 1.15 * k, -0.9 - 0.85 * k), 0.12 * k * g(k, 0.2)),
            ((-4.2, -0.75), (-4.2 - 0.7 * k, -0.8), (-4.2 - 1.3 * k, -0.85), 0.12 * k * g(k, 0.2)),
            ((-4.15, -0.6), (-4.7, -0.3), (-5.15, 0.1), 0.11 * k * g(k, 0.6))]
    return lid + low + feet + [
        ((-4.35, -2.35 - 0.2 * k), (-2.85, -3.2 - 0.45 * k), (-1.25, -2.55 - 0.2 * k), 0.30),  # brow
        ((-4.1, 0.25), (-3.1, 1.05 - 0.2 * (1 - k)), (-1.95, 0.62), 0.12 * k),              # cheek lifted
        ((-0.42, 0.65), (-1.45, 1.35), (-0.62, 2.12), 0.20),                                 # nose wing
        ((-0.62, 2.12), (-0.45, 2.3), (-0.22, 2.24), 0.10),                                  # nostril
        ((-1.3, 1.62), (-2.6 - 0.25 * k, 3.1 + 0.1 * k), (-2.4 - 0.15 * k, 4.4 + 0.3 * k), 0.14 + 0.2 * k),  # bracket
        ((-2.2 - 0.1 * k, 3.4), (-2.6, 3.6), (-2.5, 3.9 + 0.1 * k), 0.13 * k),             # corner dimple
    ]

def centre(k):
    wide = 1.7 + 0.4 * k
    out = [((-wide, 3.45), (0.0, 3.15 - 0.3 * k), (wide, 3.35), 0.30),                   # upper lip line
           ((-wide, 3.45), (0.0, 3.7 + 2.65 * k + 0.7 * HA), (wide, 3.35), 0.22 + 0.12 * k),  # the mouth opening
           ((-1.0, 4.9 + 0.55 * k), (0.0, 5.15 + 0.57 * k), (1.0, 4.87 + 0.55 * k), 0.10)]
    out += [((-1.55, 3.62), (0.0, 3.3), (1.55, 3.55), 0.10 * g(k, 0.5)),                  # edge of the teeth
            ((-0.75, 6.75 + 0.5 * HA), (0.0, 7.0 + 0.5 * HA), (0.75, 6.72 + 0.5 * HA), 0.12 * k * g(k, 0.5)),  # chin
            ((-2.6, -4.35), (0.0, -4.95), (2.6, -4.45), 0.09 * g(k, 0.6)),                # forehead
            ((-1.8, -5.15), (0.0, -5.6), (1.8, -5.2), 0.07 * g(k, 0.6))]
    return out
