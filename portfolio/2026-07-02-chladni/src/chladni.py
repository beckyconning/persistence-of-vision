...
MODES = [((7, 4), 1.0), ((5, 2), 0.55), ((9, 6), 0.35)]
SIGMA = 0.045   # width of the "sand" band around the nodal set
...
plate = np.array([0.04, 0.03, 0.09])
sand = np.array([0.98, 0.90, 0.68])
glow = np.array([0.55, 0.30, 0.62])   # faint violet halo just off the nodes
b = band[..., None]
...
write_png(os.path.join(OUT, "chladni.png"), (img * 255).astype(np.uint8))
k = 3
hh, ww = (N // k) * k, (N // k) * k
prev = img[:hh, :ww].reshape(hh // k, k, ww // k, k, 3).mean(axis=(1, 3))
write_png("/private/tmp/claude-501/-Users-beckyconning-conceptmodel/fafc4c16-3002-4c19-994d-3bed032b12f5/scratchpad/chladni_prev.png",
          (prev * 255).astype(np.uint8))
print(f"wrote chladni.png ({N}x{N}, modes={MODES})")
