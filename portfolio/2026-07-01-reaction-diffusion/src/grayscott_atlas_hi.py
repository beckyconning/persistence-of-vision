...
GW, GH = 960, 680          # native gallery resolution (no nearest-upscale)
STEPS = 15000
...
write_png(os.path.join(OUT, "grayscott_atlas_hi.png"), (img * 255).astype(np.uint8))
k2 = 4
hh, ww = (img.shape[0]//k2)*k2, (img.shape[1]//k2)*k2
prev = img[:hh, :ww].reshape(hh//k2, k2, ww//k2, k2, 3).mean(axis=(1, 3))
write_png("/private/tmp/claude-501/-Users-beckyconning-conceptmodel/fafc4c16-3002-4c19-994d-3bed032b12f5/scratchpad/rd_atlas_hi_prev.png",
          (prev * 255).astype(np.uint8))
print(f"wrote grayscott_atlas_hi.png ({GW}x{GH}, F∈[.022,.050]→x, k∈[.058,.065]→y)")
