...
NEXT (s12 left open): a piece built ENTIRELY from degrees of *translucency* (overlapping pressed petals / folded gauze) so tone comes from how much light each layer passes — true subtractive depth, no opaque blocks. And cyanotype **toning** (tea/tannin shifts blue→violet/brown) for a second hue. Cyanotype is now done; halftone-of-a-photo and the expressive full-front face remain the loudest open subjects.
