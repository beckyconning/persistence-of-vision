...
- **Descent through a self-similar point** (`the-descent.py`): per-zoom-step heightfields from the Mandeldive CPU renderer rendered with `tools/voxelspace.py`. **Lesson:** normalise each step separately to show self-similarity (the land re-forms); a global normalisation shows the escape counts climbing with depth instead (the land sinks under a fixed sea).
- **voxelspace.py v2** (same session): bilinear height sampling + `np.geomspace` depth steps. **Lesson:** near-camera stair-steps in a voxel caster come mainly from linear depth stepping (one step spans many screen rows up close), not from nearest sampling.
- **Histogram sonification with a listening rule** (`the-late-tail.py`): stdlib `wave` + numpy; buckets as fifth-stacked partials (sqrt share loudness), the rule's listened bucket as 7 Hz tremolo, decision ticks, post-decision tail at 12% amplitude; verified by an STFT spectrogram rendered to PNG in the same script (the maker cannot listen).
